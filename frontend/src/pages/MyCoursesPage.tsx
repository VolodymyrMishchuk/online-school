    ...
    if (coursesLoading) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-brand-primary font-medium">{t('dashboard.loading', 'Завантаження...')}</div>
            </div>
        );
    }
        ...
        <div className="container mx-auto px-6 py-12">
            ...
            {enrolledCourses.length === 0 ? (
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-16 text-center">
                    ...
                    <h3 className="text-2xl font-bold text-brand-dark mb-2">{t('myCourses.startLearning', 'Почніть своє навчання')}</h3>
                    <p className="text-gray-500 max-w-md mx-auto mb-8">
                        {t('myCourses.noCourses', 'Ви ще не записалися на жоден курс. Перегляньте каталог і знайдіть курс, який вас надихне.')}
                    </p>
                    <button
                    ...
                    >
                        {t('myCourses.viewCourses', 'Переглянути курси')}
                    </button>
                </div>
            ) : (
